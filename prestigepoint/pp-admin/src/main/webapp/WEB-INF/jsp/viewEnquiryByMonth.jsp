...
<body>
  <div class="container clearfix">
    ...
    <form:form method="POST" action="getEnquiryDetails.do" modelAttribute="Enquiry" autocomplete="off">
        <table width="100%" border="0">
        <label>
                  <p>
                  <h3 style="color: #873d80;">Please select atleast one for enquiry details</h3> <small class="required"></small>
                  </p> </label> 
         <%--  <tr>
          ...
          <tr>
            <td><div class="form-control">
                <label>
                  ...
                  </p><form:select path="year" class="inputControl1">
										<form:option value="0" label="SELECT" />
										<c:forEach items="${year}" var="refYear">
											<form:option value="${refYear.year}"
												label="${refYear.year}" />
										</c:forEach>
									</form:select>
										</label>
      ...
      <display:table name="enquirylist" pagesize="12" class="basic-table" uid="cat"  requestURI="getEnquiryDetails.do">
    <c:set var="count" value="${count+1}" scope="page" />
      <display:column title="S.NO" class="showHeading" style="width:1%;">${count}</display:column> 
    <c:set var="displayTotalEnquiry" value="${displayTotalEnquiry+cat[0]}" scope="page" />
    <c:set var="displayConvertedEnquiry" value="${displayConvertedEnquiry+cat[2]}" scope="page" />
    <c:set var="displayPositiveEnquiry" value="${displayPositiveEnquiry+cat[0]-cat[2]}" scope="page" />
    <fmt:parseDate var="dateObj" value="${cat[1]}" type="DATE" pattern="yyyy-MM-dd"/>
    <fmt:formatDate var="monthObj" value="${dateObj}" type="DATE" pattern="MMMM yyyy"/>
    	<display:column title="MONTH">${monthObj}</display:column>
    <c:if test="${cat[3]!=null}">
        <display:column  title="HNDELED BY"> ${cat[3]}</display:column></c:if>
		...
		<tr><th >Total:</th><td ><b>${displayTotalEnquiry}</b></td>
		<th >Converted:</th><td ><b>${displayConvertedEnquiry}</b></td>
		<th >Pending:</th><td ><b>${displayPositiveEnquiry}</b></td>
		</tr>
		</table>
      </div>
  </div>
